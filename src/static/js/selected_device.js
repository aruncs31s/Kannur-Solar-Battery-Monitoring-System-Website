  ...
  if (document.getElementById("chart-battery")) {
    ...
    function updateOldCharts() {
      for (let i = 1; i <= 6; i++) {
        fetch(`/api/data/old?device_id=${current_node}&day=${i}`)
          ...
          .catch((error) => console.error(`Error fetching data for prev_${i}:`, error));
      }
    }
    ...
    setInterval(() => {
      ...
      fetch(`/api/data?device_id=${current_node}&latest=true`) // Assuming an endpoint for latest data point exists or adjust as needed
        ...
        .then((latest) => { // Assuming the endpoint returns a single latest object or an array with one item
          if (latest && latest.hasOwnProperty("battery_voltage")) { // Adjust check if response is an array
            const x = new Date(latest.timestamp).getTime() + new Date().getTimezoneOffset() * -60000; // Use timestamp from data
            const y = parseFloat(latest.battery_voltage);

            // Add point to the live chart (today's chart)
            if (charts.battery.series[0].data.length > 4000) { // Keep shift logic if needed
